 ...
 *
 * Missouri Open Data — US/Canada government open data (data.mo.gov) via the Socrata SoQL API: state government, public safety, health & spending. Keyless.
 * Use `datasets` (catalog search) -> `metadata` (schema) -> `query` (SoQL).
 */


const BASE = 'https://data.mo.gov';
const UA = 'pipeworx-mcp-data-mo/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [
  {
    name: 'datasets',
    description: 'Search the Missouri Open Data catalog of open datasets by keyword. Returns each dataset\'s resource_id, name, description, category and update date — pass the resource_id to query/metadata.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'query',
    description: 'Run a Socrata SoQL query against a Missouri Open Data dataset by resource_id (e.g. "gfq7-aa86"). Filter with where/select/group/order (SoQL clauses, without the leading $) plus limit/offset. Returns matching rows as JSON.',
    inputSchema: {
      type: 'object',
      properties: {
        resource_id: { type: 'string', description: 'Dataset id, e.g. "gfq7-aa86" (from datasets).' },
  ...
  {
    name: 'metadata',
    description: 'Get a Missouri Open Data dataset\'s schema + metadata (columns, types, row count, category, last-updated) by resource_id, e.g. "gfq7-aa86".',
    inputSchema: {
      type: 'object',
      properties: { resource_id: { type: 'string', description: 'Dataset id, e.g. "gfq7-aa86".' } },
      required: ['resource_id'],
    },
  },
];
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case 'query': {
      const id = reqStr(args, 'resource_id', '"gfq7-aa86"');
      const p = new URLSearchParams();
    ...
    case 'metadata':
      return socrataGet(`/api/views/${reqStr(args, 'resource_id', '"gfq7-aa86"')}.json`);
    default:
...
async function socrataGet(path: string): Promise<unknown> {
  ...
  if (!res.ok) throw new Error(`data.mo.gov: ${res.status}`);
  return res.json();
}
